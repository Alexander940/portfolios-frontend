 ...
 * RelevantEventsList
 *
 * Shared presentational body for relevant events (rating upgrades/downgrades +
 * price movers). Renders loading skeletons, an error row with retry, an empty
 * state, and the event rows themselves (rating events flat; movers grouped into
 * Gainers / Losers). Used by both the RelevantEventsRail (list-view panel) and
 * the PortfolioEventsTab (full-width, single-portfolio scope) so the two render
 * rows identically. Callers own their own chrome (panel vs. tab) and chips.
 */
    ...
    >
      ...
      <div className="rail-row-side">
        <span className="rail-ratings">
          ...
          {delta !== null && delta !== 0 && (
            <span className={`rail-delta ${delta > 0 ? 'pos' : 'neg'}`}>
              {delta > 0 ? `+${delta}` : delta}
            </span>
          )}
        </span>
