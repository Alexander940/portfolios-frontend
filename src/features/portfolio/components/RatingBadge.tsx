 ...
 * RATING_CONFIG constant and formatRatingLabel helper it depends on, so the
 * positions table and the events rail import all three from one place. The
 * rule only affects Vite fast-refresh granularity, not correctness. */

/**
 * Shared rating UI for the portfolio feature.
 *
 * Rating domain is the integer set {-3,-2,-1,1,2,3} (no 0); higher = more
 * bullish. Positive ratings render green, negative red. Extracted from
 * PortfolioPositionsTable so the positions table and the Relevant Events rail
 * render rating badges identically.
 */
